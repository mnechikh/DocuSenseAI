...
export async function listWebhooks(): Promise<{ hooks: WebhookSummary[]; error?: string }> {
  try {
    const user = await getSessionUser();
    if (!user || user.role !== 'Admin') return { hooks: [], error: 'Only Admins can list webhooks.' };
...
export async function deleteWebhook(webhookId: string): Promise<{ error?: string }> {
  try {
    const user = await getSessionUser();
    if (!user || user.role !== 'Admin') return { error: 'Only Admins can delete webhooks.' };

    const ref = adminDb.collection('webhooks').doc(webhookId);
    const snap = await ref.get();
    if (!snap.exists) return { error: 'Webhook not found.' };
...
export async function sendTestPing(webhookId: string): Promise<{ success: boolean; statusCode: number | null; error?: string }> {
  try {
    const user = await getSessionUser();
    if (!user || user.role !== 'Admin') return { success: false, statusCode: null, error: 'Only Admins can test webhooks.' };

    const snap = await adminDb.collection('webhooks').doc(webhookId).get();
    if (!snap.exists) return { success: false, statusCode: null, error: 'Webhook not found.' };
    const webhook = snap.data() as WebhookRecord;
    if (webhook.tenantId !== user.tenantId) return { success: false, statusCode: null, error: 'Not found.' };
    ...
    try {
      ...
      return { success: false, statusCode: null, error: 'Could not reach endpoint.' };
    } finally {
      clearTimeout(timeout);
    }
  } catch (err) {
    return { success: false, statusCode: null, error: (err as Error).message };
  }
}
